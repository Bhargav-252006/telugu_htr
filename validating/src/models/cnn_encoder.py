...
def _patch_resnet18_strides(model: nn.Module, high_res_temporal: bool = False) -> nn.Module:
    ...
    # Remove stride from layer3 and layer4 completely
    for layer_name in ("layer3", "layer4"):
        layer = getattr(model, layer_name)
        first_block = layer[0]

        # Patch the main conv stride to (2, 1) to compress height but keep width
        if hasattr(first_block, "conv1"):
            first_block.conv1.stride = (2, 1)
        # Patch the downsample projection (skip connection) stride to (2, 1)
