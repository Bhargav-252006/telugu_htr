...
Our system comprises four components: (A)~a CNN feature encoder, (B)~a CTC baseline decoder, (C)~an autoregressive Transformer decoder with auxiliary CTC training, and (D)~a Telugu linguistic constraint mask. Fig.~\ref{fig:training_curves} shows the training convergence of both model families.

\begin{figure*}[t]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/training_curves.png}
    \caption{Training convergence. (a)~Validation CER (\%) vs.\ epoch for CTC Baseline and AR Transformer models, showing smooth convergence. (b)~Training loss vs.\ epoch. Both models were trained for 50 epochs on a single NVIDIA RTX 3090 Ti.}
